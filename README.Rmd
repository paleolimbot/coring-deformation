...
To calculate parameters for the deformation model, we loaded `r nrow(deformed_core_photos)` scale photos of deformed cores from `r length(unique(deformed_core_photos$reference))` sources into ImageJ software and digitized deformed strata (Table 1). Coordinates were transformed to `r` and `d` values for individual strata by subtracting the minimum `d` value from the rest of the values, and subtracting the central `x` value from the rest of the values. Power regression (quadratic) was performed on the data to obtain reasonable coefficients for minimum, maximum, and mean levels of deformation.
...
ggplot(all %>% filter(abs(y)==min(abs(y))), aes(x=x, y=z)) + geom_raster(aes(fill=d0)) + 
  ...
  labs(x="Width (cm)", y=NULL)
```

```{r, fig.cap="*Fig. 6* Distribution of d~0~ values modeled for multiple deformation coefficients and slice sizes."}
...
ggplot(histograms %>% filter(density != 0), aes(x=d, y=density)) + 
  ...
  labs(x=expression(d[0]), y="Density")
```
